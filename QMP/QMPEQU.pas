...
begin
  ...
  Result.Open := TQMPEQU.Open;
  Result.Stop := TQMPEQU.Stop;
...
begin
  for k := 0 to Length(TQMPEQU.feqz) - 1 do begin
    for i := 0 to Length(TQMPEQU.feqz[k]) - 1 do begin
      TQMPEQU.feqz[k, i].Init(bqfEqu, bqfOctave, bqfDb);
...
class function TQMPEQU.Open(const Media: PChar; const Format: Pointer; const Flags: Integer): Integer; cdecl;
var
  k: LongWord;
  i: LongWord;
begin
...
class procedure TQMPEQU.Stop(const Flags: Integer); cdecl;
var
  k: LongWord;
  i: LongWord;
begin
