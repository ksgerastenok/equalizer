...
begin
  ...
  if (self.fenabled) then begin
    for k := 0 to self.fdsp.Data.Channels - 1 do begin
      ...
      for x := 0 to self.fdsp.Data.Samples - 1 do begin
        if (b <= a) then begin
          y := b * ((x / self.fdsp.Data.Rates) * (a - (1.0 + 0.01) * b) + 0.01 * a * 0.3) / ((x / self.fdsp.Data.Rates) * (a - (1.0 + 0.01) * b) + 0.01 * b * 0.3);
        end         else begin
          y := b * ((x / self.fdsp.Data.Rates) * (a - (1.0 - 0.01) * b) - 0.01 * a * 9.0) / ((x / self.fdsp.Data.Rates) * (a - (1.0 - 0.01) * b) - 0.01 * b * 9.0);
        end;
        y := Min(Max(1.0, y), 10.0);
