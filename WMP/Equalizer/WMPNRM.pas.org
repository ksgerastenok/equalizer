...
procedure TWMPNRM.Init(const Transform: TTransform; const Filter: TFilter; const Band: TBand; const Gain: TGain);
begin
...
function TWMPNRM.getBand(): TBand;
begin
...
function TWMPNRM.getFilter(): TFilter;
begin
...
function TWMPNRM.getFreq(): Double;
begin
  Result := self.fbqf.Freq;
end;

procedure TWMPNRM.setFreq(const Value: Double);
begin
...
function TWMPNRM.getWidth(): Double;
begin
  Result := self.fbqf.Width;
end;

procedure TWMPNRM.setWidth(const Value: Double);
begin
  self.fbqf.Width := Value;
end;

procedure TWMPNRM.addSample(const Value: Double);
begin
  self.fsqr := self.fsqr - (self.fsqr - Sqr(Value)) / IfThen(Sqr(self.fval * Value) < 1.0, 5.0 * self.fbqf.Rate, 0.5 * self.fbqf.Rate);
  self.favg := self.favg - (self.favg - Abs(Value)) / IfThen(Abs(self.fval * Value) < 1.0, 5.0 * self.fbqf.Rate, 0.5 * self.fbqf.Rate);
