...
procedure TWMPFRM.Init(var Info: TInfo);
begin
  self.finfo := Addr(Info);
  self.Create();
end;
...
begin
  ...
  self.CreateTrackBar(5, 5, 20, 255, 99);
  self.CreateStaticText(245, 30, 45, 20, '-20 dB');
  self.CreateStaticText(125, 40, 45, 20, '0 dB');
  self.CreateStaticText(5, 30, 45, 20, '+20 dB');
  for i := 0 to Length(self.finfo.Bands) - 1 do begin
    self.CreateTrackBar(5, 85 + 30 * i, 20, 255, i);
    self.CreateStaticText(265, 80 + 30 * i, 30, 20, IfThen(20 * Power(2, 0.5 * i) < 1000, Format('%4.0f ', [20 * Power(2, 0.5 * i) / 1]), Format('%4.1fk', [20 * Power(2, 0.5 * i) / 1000])));
  end;
end;
