...
begin
  ...
  if((Sender is TCheckBox)) then begin
    if(((Sender as TCheckBox).Tag = 99)) then begin
      (Sender as TCheckBox).Checked := TWMPEQU.Info.Enabled;
  ...
  if((Sender is TTrackBar)) then begin
    if(((Sender as TTrackBar).Tag = 99)) then begin
      (Sender as TTrackBar).Position := (Sender as TTrackBar).Max - TWMPEQU.Info.Preamp                           + (Sender as TTrackBar).Min;
      ...
      (Sender as TTrackBar).Position := (Sender as TTrackBar).Max - TWMPEQU.Info.Bands[(Sender as TTrackBar).Tag] + (Sender as TTrackBar).Min;
      (Sender as TTrackBar).Hint := Format('Gain: %s dB', [FloatToStr(((Sender as TTrackBar).Max - (Sender as TTrackBar).Position + (Sender as TTrackBar).Min) / 10)]);
    end;
  end;
end;
...
begin
  ...
  if((Sender is TCheckBox)) then begin
    if(((Sender as TCheckBox).Tag = 99)) then begin
      TWMPEQU.Info.Enabled := (Sender as TCheckBox).Checked;
      (Sender as TCheckBox).Hint := Format('Enabled: %s', [BoolToStr((Sender as TCheckBox).Checked, True)]);
    end                                  else begin
      TWMPEQU.Info.Enabled := (Sender as TCheckBox).Checked;
      (Sender as TCheckBox).Hint := Format('Enabled: %s', [BoolToStr((Sender as TCheckBox).Checked, True)]);
  ...
  if((Sender is TTrackBar)) then begin
    if(((Sender as TTrackBar).Tag = 99)) then begin
      TWMPEQU.Info.Preamp                           := (Sender as TTrackBar).Max - (Sender as TTrackBar).Position + (Sender as TTrackBar).Min;
      (Sender as TTrackBar).Hint := Format('Gain: %s dB', [FloatToStr(((Sender as TTrackBar).Max - (Sender as TTrackBar).Position + (Sender as TTrackBar).Min) / 10)]);
    end                                  else begin
      TWMPEQU.Info.Bands[(Sender as TTrackBar).Tag] := (Sender as TTrackBar).Max - (Sender as TTrackBar).Position + (Sender as TTrackBar).Min;
      (Sender as TTrackBar).Hint := Format('Gain: %s dB', [FloatToStr(((Sender as TTrackBar).Max - (Sender as TTrackBar).Position + (Sender as TTrackBar).Min) / 10)]);
    end;
  end;
end;
